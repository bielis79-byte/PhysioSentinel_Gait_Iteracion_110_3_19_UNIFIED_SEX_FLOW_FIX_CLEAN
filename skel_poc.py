...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    for fi in range(int(start_index),len(frames)):
        ...
        if fi>int(start_index):
            ...
            pose,lower_temporal=_pose_sequence_integrity_lower_update_147(torch,model,pose,rotvec,trans,scale,tdf,rows,betas,zero_trans,prev_pose,axis_map,lower_profile,acquisition_mode)
        with torch.no_grad():
            world,_,_=_direct_world_130(torch,model,pose,betas,zero_trans,rotvec,scale,trans,axis_map=axis_map)
    ...
    result={'version':'110.3.19','retargeting_mode':'official_skel24_pose_sequence_integrity_fix','scale':float(scale.cpu()),
...
def _plot_skel_mesh_walker(mesh_seq):
    ...
    try:
        ...
        const layout={{title:'V110.3.18 · SKEL ANIMATED ANATOMICAL MESH · pelvis-centered playback',margin:{{l:0,r:0,t:45,b:0}},scene:{{aspectmode:'data',xaxis:{{range:P.ranges[0]}},yaxis:{{range:P.ranges[1]}},zaxis:{{range:P.ranges[2]}}}},legend:{{orientation:'h'}}}};
        ...
        st.caption("V110.3.18 · visor centrado en pelvis sólo para presentación. El NPZ científico conserva las coordenadas originales; el contador debe avanzar al pulsar ▶.")
    except Exception as exc:
...
def _patient_gender_145():
    """V110.3.19 · única fuente de verdad: Sexo visible de la ficha general.

    Prioriza el widget general vivo y sincroniza el valor canónico para que SKEL nunca
    conserve un sexo antiguo ni elija male/female por defecto.
    """
    raw = st.session_state.get("patient_sex_live_widget", st.session_state.get("patient_sex", "No especificado"))
...
def _mesh_sequence_from_npz_bytes_145(raw: bytes):
    ...
    with np.load(io.BytesIO(raw),allow_pickle=False) as z:
        ...
        out={
            "version":str(scalar("version","110.3.19-import")),
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.19 separa reproducción y recálculo. Un NPZ antiguo puede reproducirse tal cual o usarse sólo como referencia mientras el motor actual recalcula la marcha desde los 75 frames V104/V107 cargados en esta sesión.")
    ...
    if _npz is not None:
        try:
            ...
            if _mode=='play':
                ...
                st.download_button("⬇️ Reexportar marcha SKEL cargada (NPZ)",_mesh_sequence_npz_bytes(_loaded),"V110_3_18_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
                return
            elif _mode=='recalc':
    ...
    st.markdown("**Modelo SKEL privado · V110.3.19 CPU + Lower-Limb Temporal Retargeting + Pelvis-Centered Playback + Unified Sex + B2 Fix**")
        ...
        with st.spinner("Instanciando SKEL y ejecutando el primer forward CPU (1 frame)…"):
            ...
            try:
                ...
                try:
                    ...
                    export={
                        "version":"110.3.19","frame":int(best_i)+1,"gender":gender,"rmse_before":fit["rmse_before"],"rmse_after":fit["rmse_after"],
                    ...
                    st.markdown("### V110.3.18 · Propagación temporal 1→75")
                    ...
                    if st.button("🧍▶️ Procesar 75 frames + generar malla anatómica SKEL",type="primary",use_container_width=True,key="v110_3_18_run_sequence",disabled=not frame_gate_ok):
                        ...
                        try:
                            ...
                            if len(oks_auto)>=max(2,int(0.9*expected)) and bool(_ll_gate.get('ok',False)):
                                ...
                                bar.progress(1.0,text="Secuencia temporal NO válida para malla")
                                if not bool(_ll_gate.get('ok',False)):
                                    st.error("⛔ TEMPORAL MOTION GATE: los miembros inferiores del objetivo se mueven pero SKEL no reproduce movimiento suficiente. La malla NO se genera para evitar una falsa marcha.")
                                    st.json(_json_safe_134(_ll_gate))
                                else:
                                    st.warning("Secuencia parcial: malla automática omitida.")
                            status.empty()
                        except Exception as seq_exc:
                            st.session_state.pop("v110_3_18_sequence",None)
                            st.session_state.pop("v110_3_18_mesh_sequence",None)
                            st.session_state.pop("v110_3_18_mesh_audit",None)
                    ...
                    if isinstance(seq,dict) and seq.get("frames"):
                        ...
                        st.download_button("⬇️ Descargar secuencia SKEL V110.3.13 (JSON)",json.dumps(_json_safe_134(export_seq),ensure_ascii=False).encode("utf-8"),"V110_3_18_SKEL_sequence.json","application/json",use_container_width=True)
                        ...
                        st.markdown("### V110.3.18 · SKEL ANIMATED ANATOMICAL MESH")
                        ...
                        if isinstance(mesh_seq,dict) and isinstance(mesh_seq.get("vertices"),np.ndarray):
                            ...
                            st.download_button("⬇️ Descargar SKEL Mesh Walker V110.3.18 (NPZ)",_mesh_sequence_npz_bytes(mesh_seq),"V110_3_18_SKEL_mesh_sequence.npz","application/octet-stream",use_container_width=True)
